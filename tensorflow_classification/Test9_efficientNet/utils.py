...
def generate_ds(data_root: str,
    ...
    def configure_for_performance(ds, shuffle_size: int, shuffle: bool = False):
        ds = ds.cache()  # 读取数据后缓存至内存
         # Verifique se o tamanho do conjunto de dados é maior que zero
        #assert total_train > 0, "The size of the dataset must be greater than zero."
        # Verifique e ajuste o tamanho do buffer de shuffle
        # ds = ds.shuffle(buffer_size=max(1, shuffle_size))  # Adicione max(1, ...) para garantir um tamanho mínimo de 1
